...
public class MageController : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity, mouseAimMask))
        {
            ...
            Vector3 phoneAcc = m_gyro.userAcceleration;
            Vector3 phoneRot = m_gyro.rotationRateUnbiased;

            RpcSetTarget(phoneAcc, phoneRot);
    ...
    [ClientRpc]
    private void RpcSetTarget(Vector3 phoneAcc, Vector3 phoneRot)
    {
        

        Vector3 UnityAcc = new Vector3(phoneAcc.x, -phoneAcc.y, phoneAcc.z);
        Vector3 UnityRot = new Vector3(phoneRot.x, phoneRot.y, -phoneRot.z);

        targetTransform.position += UnityAcc * 10;
        targetTransform.position += UnityRot * 10;
    }
